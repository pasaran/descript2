...
de.Block.prototype._do_phase_error = function *( params, context, state, error ) {
    ...
    //  FIXME: Можно предусмотреть `options.is_error`.
    //  А не вызвыать ли тут done_in_context?
    if ( this.options.required && error.error.id !== 'REQUIRED_BLOCK_FAILED' ) {
        error = de.error( {
    ...
    if ( this.options.error ) {
        if ( Array.isArray( this.options.error ) ) {
            for ( var i = 0; i < this.options.error.length; i++ ) {
                this.options.error[ i ]( params, context, state, error );
            }

        } else {
            this.options.error( params, context, state, error );
        }
    }
