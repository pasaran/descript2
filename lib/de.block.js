...
Block.Http._extend_block = function( what, by ) {
    ...
    de.request( options, context )
        .then(
            function( result ) {
                ...
                //  FIXME: Тут как-то тщательнее нужно. Может быть 'application/json-foo', например.
                var is_json = block.is_json || ( result.headers[ 'content-type' ].substr( 0, 16 ) === 'application/json' );
                var body;
                if ( !result.body ) {
                    body = null;
