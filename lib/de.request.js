...
de.request = function( options, logger ) {
    var start = Date.now();
    ...
    function do_resolve( result ) {
        ...
        var end = Date.now();
        logger.log( `[${ request_id }] Request ended in ${ end - start }ms [${ options.method }] ${ full_url }` );
    ...
    function do_reject( result ) {
        ...
        var end = Date.now();
        logger.error( `[${ request_id }] Request ended in ${ end - start }ms with an error ${ JSON.stringify( result ) }` );
    ...
    function do_retry( options ) {
        ...
        logger.debug( `[${ request_id }] Retry #${ options.retries }` );

        do_request( options );
    }

    function do_redirect( options, location ) {
        logger.debug( `[${ request_id }] Redirected to ${ location }` );
