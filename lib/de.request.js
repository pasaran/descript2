...
de.request = function( options, context ) {
    ...
    function do_request( request_options ) {
        ...
        const request_handler = function( httpRes ) {
            const res = decompress_response( httpRes );
            ...
            res.on( 'end', function() {
                ...
                if ( !httpRes.complete ) {
                    const error = de.error( {
                        id: de.Error.ID.INCOMPLETE_RESPONSE,
                    } );
...
de.request.DEFAULT_OPTIONS = {
    ...
    is_retry_allowed: function( error, request_options ) {
        if ( error.id === de.Error.ID.TCP_CONNECTION_TIMEOUT ) {
            return true;
        }
