        ...
        for ( var j = 0, m = group.length; j < m; j++ ) {
            ...
            promise.then( function( block_result ) {
                ...
                if ( de.is_error( block_result ) ) {
                    if ( block_result.error.id === de.Error.ID.REQUIRED_BLOCK_FAILED ) {
                        running.resolve( block_result );
                        running.trigger( 'abort', block_result );

                    } else if ( block.options.required ) {
