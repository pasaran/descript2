        ...
        action: function( params, context ) {
            ...
            if ( deps ) {
                ...
                for ( var i = 0, l = deps.length; i < l; i++ ) {
                    ...
                    promises.push( context._get_promise( dep_id ) );
                }

                context._dependent_blocks[ this.get_id() ] = true;
                context._queue_deps_check();

                //  FIXME: Странный хак. Массив с результатами блоков из deps
                //  приходит как progress и портит результат.
                //  Может проверять action.length и игнорировать progress, если что.
                //
                var that = this;
...
de.Block.prototype.error = function( error, context ) {
    var promise = context._get_promise( this.get_id() );
