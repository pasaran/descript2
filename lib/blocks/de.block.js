    ...
    after: {
        ...
        action: function( params, context, progress ) {
            ...
            if ( this.options.after ) {
                return run_callbacks_chain( no.array( this.options.after ), params, context );
            }
        },
...
de.Block.prototype._run_phases = function( params, context ) {
    ...
    function run( phase ) {
        ...
        if ( no.is_promise( running ) ) {
            return running.then( on_done, on_fail );
        }

        on_done( running );

        function on_done( value ) {
            var callback;
            if ( de.is_error( value ) ) {
                callback = phase.fail;
            }

            do_done( callback || phase.done, value );
        }

        function on_fail( value ) {
