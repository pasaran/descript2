...
de.Block.prototype._init_options = function( options ) {
    ...
    var dir_options;
    if ( options.dirname ) {
        dir_options = load_dir_options( options.dirname );
    }
    if ( dir_options ) {
        //  FIXME: Или кидать тут ошибку?
        delete dir_options.id;

        options = de.Block.extend_options( dir_options, options );
    }
    //  FIXME: Нужен ли тут extend? В конструкторе lazy-блоков это уже делается.
