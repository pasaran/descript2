...
de.Block.prototype._do_phase_before = function *( params, context, state ) {
    ...
    if ( this.options.before ) {
        error = yield run_callbacks_chain( no.array( this.options.before ), params, context, state );
    }
    if ( de.is_error( error ) ) {
        return yield *this._do_phase_error( params, context, state, error );
    }
    ...
    this._done_in_context( params, context, state );
    ...
    yield *this._do_phase_after( params, context, state, progress );
};
...
de.Block.prototype._do_phase_after = function *( params, context, state, progress ) {
    ...
    if ( this.options.after ) {
        error = yield run_callbacks_chain( no.array( this.options.after ), params, context, state, progress );
    }
