...
de.Block.prototype._params = function( params, jvars ) {
    ...
    if ( typeof o_params === 'function' ) {
        return o_params( params, params, jvars );
    }
    ...
    for ( var p_name in o_params ) {
        ...
        if ( p_value == null ) {
            ...
            new_params[ p_name ] = ( typeof p_value === 'function' ) ? p_value( params, params, jvars ) : p_value;
        }
    }
...
de.Block.prototype._do_phase_before = function *( params, context, state ) {
    ...
    if ( deps ) {
        //  FIXME: Может быть это делать вообще всегда?
        ...
        for ( var i = 0, l = deps.length; i < l; i++ ) {
            ...
            dep_promise.then( function( result ) {
                if ( de.is_error( result ) ) {
                    ...
                    //  FIXME: Это нужно делать не здесь,
                    //  а только после того, как все закончилось.
                    //  Или все ок? Пока все промисы не отработают,
                    //  новый стейт все равно нигде использоваться не будет.
                    if ( dep.select ) {
                        do_select( dep.select, result, params, context, state );
                    }
                }
            } );
    ...
    var jvars = { context, state };

    var key;
    if ( this.options.key ) {
        key = this.options.key( params, params, jvars );
        if ( key ) {
    ...
    params = this._params( params, jvars ) || params;
...
de.Block.prototype._do_phase_after = function *( params, context, state, progress ) {
    ...
    if ( this.options.select ) {
        do_select( this.options.select, progress, params, context, state );
    }

    if ( this.options.result ) {
        //  FIXME: Если это jpath, то нужно как-то запихнуть state, context, params в jvars.
        progress = yield this.options.result( progress, { params, context, state } );
    }
...
function do_select( select, result, params, context, state ) {
    var jvars = { params, context, state };
    for ( var name in select ) {
        var value = select[ name ]( result, result, jvars );
...
function async_iterate( it, context ) {
    return new Promise( function( resolve ) {
        ( function run( value ) {
            ...
            if ( context._is_aborted ) {
                resolve( de.error( 'ABORTED' ) );
            }
...
de.Block.prototype._run = function( params, context ) {
    ...
    async_iterate( this._do_phase_before( params, context, {} ), context )
