...
de.Block.prototype._init_options = function() {
    ...
    options.id = raw_options.id;

    options.deps = raw_options.deps || null;
...
var phases = {
    deps: {
        ...
        action: function( params, context, progress ) {
            ...
            console.log( 'phase.deps', this.id, this._type, deps );
            if ( deps ) {
                var promises = [];
                ...
                context._waiting_for_n_deps[ this.id ] = promises.length;
    ...
    before: {
        ...
        action: function( params, context, progress ) {
            console.log( 'phase.before', this.id, this._type, progress );
    ...
    after: {
        ...
        action: function( params, context, progress ) {
            //  console.log( 'phase.after', this.id, this._type );
...
de.Block.prototype._run_phases = function( start_phase, params, context ) {
    var promise = context._get_promise( this.id );
...
de.Block.prototype._done_in_context = function( context ) {
    ...
    var ids = context._required_as_dep_by[ this.id ];
    for ( var id in ids ) {
    ...
    var ids = context._required_as_subblock_by[ this.id ];
    for ( var id in ids ) {
    ...
    if ( context._total_blocks !== 0 && context._n_active_blocks === 0 && next === 0 ) {
