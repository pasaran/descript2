...
var phases = {
    deps: {
        ...
        action: function( params, context ) {
            ...
            //  console.log( 'phase.deps', this.get_id(), this._type, deps );
            if ( deps ) {
                var promises = [];
                for ( var i = 0, l = deps.length; i < l; i++ ) {
                    ...
                    if ( promise.is_pending() ) {
                        ...
                        var ids = context._required_as_dep_by[ id ] || (( context._required_as_dep_by[ id ] = {} ));
                        ids[ this.get_id() ] = true;
                    }
                }

                context._waiting_for_n_deps[ this.get_id() ] = promises.length;

                context.print();

                if ( promises.length && !context._n_active_blocks ) {
                    return de.error( 'Deps cannot be resolved' );
                }

                if ( promises.length ) {
                    //  FIXME: Тут нужно новый promise возвращать, чтобы в before не приходил progress.
                    return no.promise.all( promises ).then(
                        function() {}
                    );
                }
            }
    ...
    action: {
        ...
        action: function( params, context, progress ) {
            ...
            running = this._do_action( params, context );
...
de.Block.prototype.run = function( params, context ) {
    ...
    //  console.log( 'run', this.id, this._type );
    context._add_block( this );

    return this._run_phases( phases.deps, params, context );
};
...
de.Block.prototype._start_in_context = function( context ) {
    ...
    context._queued_in_next_tick = false;
...
de.Block.prototype._done_in_context = function( context ) {
    ...
    if ( context._active_blocks[ this.get_id() ] ) {
        context._n_active_blocks--;
    }
    context._total_blocks--;
    //  console.log( '_done_in_context', this.id, this._type );

    var next = 0;

    var ids = context._required_as_dep_by[ this.get_id() ];
    for ( var id in ids ) {
    ...
    var ids = context._required_as_subblock_by[ this.get_id() ];
    for ( var id in ids ) {
    ...
    //  FIXME: Странный костыль.
    //
    //  У нас после завершения какого-то блока может быть блок,
    //  в котором сошлись зависимости и блок, в котором завершились
    //  все подблоки:
    //
    //      {
    //          foo: [ b1, b2 ], // b3
    ...
    //      }
    //
    //  Сперва, скажем, завершился b1, потом b2. При завершении b2, b4 можно запустить,
    //  но это случится только в next tick'е. А b3 заканчивается в этом же.
    //  А счетчики уже уменьшились, так что запускать вроде и нечего.
    //
    if ( next ) {
        context._queued_in_next_tick = true;
    }

    //  console.log( 'NEXT', next );
    context.print();

    if ( !context._queued_in_next_tick && context._total_blocks !== 0 && context._n_active_blocks === 0 && next === 0 ) {
