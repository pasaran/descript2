...
de.Block.Http.prototype._init_extra_options = function() {
    ...
    if ( raw_options.only_meta ) {
        this.options.only_meta = true;
    }

    if ( raw_options.is_json ) {
        this.options.is_json = true;
    }
};
    ...
    de.request( options, context )
        .then(
            function( result ) {
                ...
                var is_json = that.is_json || ( result.headers[ 'content-type' ] === 'application/json' );
                var body;
                if ( !result.body ) {
                    body = null;
